...
const ProfileSettingsModal: React.FC<ProfileSettingsModalProps> = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        if (profile) {
            ...
            setPhone(profile.phone || ''); // Atualiza o estado do telefone
        } else if (session) {
            // Preenche com dados padrão se o perfil ainda não existir
            setFullName('');
            setRole('');
    ...
    const handleSave = useCallback(async () => {
        ...
        const success = await updateProfile({
            ...
            phone: phone.trim(), // Inclui o telefone no objeto de atualização
        });
    ...
    const handleAvatarUpdate = useCallback(async (newUrl: string) => {
        ...
        // O fetchProfile já é chamado dentro do updateProfile, então não precisamos chamar de novo.
        return success;
    }, [updateProfile]);
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <TextInput 
                            ...
                            id="phone" 
                            value={phone} 
                            onChange={(e) => setPhone(e.target.value)} // Campo agora é editável
