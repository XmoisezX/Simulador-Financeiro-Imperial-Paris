...
const ResultsTable: React.FC<ResultsTableProps> = ({ monthlyData, totals, taxRate }) => {
    const headers = [
        "Mês", "Nº Vendas", "Nº Aluguéis", "Fat Bruto Venda", "Fat Bruto Alug (1º)", "Fat Bruto Alug (Adm)", "Fat Bruto Total",
    ...
    const tooltips: { [key: string]: string } = {
        ...
        "Com Var Venda (C)": "Comissão total paga aos corretores externos sobre as vendas que eles realizaram (venda + agenciamento).",
        "Com Var Alug (1º S)": "Comissão variável paga aos sócios sobre os novos contratos de aluguel.",
        "Rec Líquida (p/ CF)": "Receita Líquida: Faturamento Bruto Total subtraindo todos os custos variáveis (impostos, comissões, etc). Valor disponível para cobrir os custos fixos.",
        ...
        "Margem Contrib.": "Margem de Contribuição Percentual. (Receita Líquida / Faturamento Bruto Total). Mostra quanto % da receita sobra para pagar custos fixos e gerar lucro.",
        "Lucratividade Op.": "Lucratividade Operacional Percentual. (Lucro Operacional / Faturamento Bruto Total). Mostra a eficiência da operação principal em gerar lucro.",
